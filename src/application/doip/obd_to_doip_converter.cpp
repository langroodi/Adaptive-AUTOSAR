...
namespace application
{
    namespace doip
    {
            ...
            std::vector<uint8_t> &&receivedUdsData) const
        {
            ...
            bool cSuccessful{
                tryParseUdsData(std::move(receivedUdsData), _obdData)};

            if (cSuccessful && mCallback)
            {
                mCallback(std::move(_obdData));
            }
        }
        ...
        bool ObdToDoipConverter::TryGetResponseAsync(
            const std::vector<uint8_t> &pid,
            std::function<void(std::vector<uint8_t> &&)> &&callback)
        {
            ...
            case 0x4f:
                ...
                callback({0x02, 0x08, 0x80, 0x19});
                break;
            case 0x50:
                // Maximum mass air flow rate is 650 g/s.
                callback({0x41, 0x00, 0x00, 0x00});
                break;
            case 0x51:
            ...
            case 0x05:
            case 0xa6:
                // Set the callback if it has not been set before
                // It is assumed that the callback be the same for different PIDs
                if (!mCallback)
                {
